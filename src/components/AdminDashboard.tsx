...
import { LayoutGrid, LogOut, Settings2, Clock, CheckCircle2 } from 'lucide-react';
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 md:p-8">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {/* Option 1: Pendência Completo (Módulo Principal) */}
                    <motion.button
                        whileHover={{ y: -6 }}
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        onClick={() => onSelectModule('complete')}
                        className="group relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-8 md:p-12 text-left transition-all hover:border-indigo-500 shadow-xl shadow-slate-200/50 dark:shadow-none overflow-hidden h-full flex flex-col justify-between"
                    >
                        ...
                        <div className="flex items-center gap-4 py-4 px-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                            ...
                            <div className="flex items-center gap-2 text-slate-500 text-sm font-bold">
                                <Clock className="w-4 h-4" /> Painel Principal
                            </div>
                        </div>
                    </motion.button>

                    {/* Option 2: Configurações */}
                    <motion.button
                        whileHover={{ y: -6 }}
                        initial={{ opacity: 0, x: 20 }}
                        animate={{ opacity: 1, x: 0 }}
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4 py-4 px-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                            <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 text-sm font-black uppercase tracking-widest">
                                Acessar Módulo
                            </div>
                        </div>
                    </motion.button>
