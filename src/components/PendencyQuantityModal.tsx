        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-6">
                ...
                <motion.div
                    ...
                    className="relative w-full max-w-lg md:max-w-4xl max-h-[94dvh] bg-white dark:bg-slate-900 rounded-2xl md:rounded-[2rem] shadow-2xl flex flex-col overflow-hidden transition-all duration-300"
                >
                    {/* Banner de Topo - Compacto e Elegante */}
                    <div className="bg-amber-500 p-3 md:p-4 flex items-center justify-between text-white shadow-md relative z-20 shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl overflow-hidden shadow-lg border border-white/30 bg-white">
                    ...
                    <div className="flex flex-col md:flex-row overflow-y-auto">
                        ...
                        <div className="w-full md:w-[45%] flex flex-col items-center bg-slate-50 dark:bg-slate-950/20 border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-800">
                            {/* Imagem da Roda */}
                            <div className="flex items-center justify-center p-3 md:p-6 relative">
                                <motion.img 
                                    layoutId={`photo-${item.codigo}`}
                                    src={photoUrl || "https://placehold.co/400x400/e2e8f0/64748b?text=FOTO"} 
                                    alt={`Foto ${item.codigo}`} 
                                    className="w-24 h-24 sm:w-32 sm:h-32 md:w-64 md:h-64 object-contain rounded-full bg-white shadow-xl border-4 border-white dark:border-slate-800"
                                />
                            </div>

                            <div className="px-3 pb-3 md:p-6 md:pt-0 w-full flex flex-col gap-4">
                                <div className="bg-white dark:bg-slate-800/40 p-3 md:p-5 rounded-2xl md:rounded-3xl border border-slate-200 dark:border-slate-700/50 text-center shadow-sm">
                                    <h3 className="text-sm md:text-xl font-black text-slate-800 dark:text-slate-100 leading-tight mb-3 md:mb-4 line-clamp-2">
                                        {item.descricao}
                                    </h3>
                                    
                                    <div className="flex items-center justify-center gap-2 md:gap-4">
                                        <div className="flex flex-col items-center bg-slate-50 dark:bg-slate-900 px-3 md:px-4 py-2 rounded-xl md:rounded-2xl border border-slate-100 dark:border-slate-800 w-24 md:w-28">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Estoque</span>
                                            <span className="text-base md:text-lg font-bold text-slate-700 dark:text-slate-200">{stockQty}</span>
                                        </div>
                                        <div className="flex flex-col items-center bg-slate-50 dark:bg-slate-900 px-3 md:px-4 py-2 rounded-xl md:rounded-2xl border border-slate-100 dark:border-slate-800 w-24 md:w-28">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Pendência</span>
                                            <span className="text-base md:text-lg font-bold text-slate-700 dark:text-slate-200">{pendencyQty}</span>
                                        </div>
                                    </div>

                                    <div className="mt-3 md:mt-4 pt-3 md:pt-4 border-t border-slate-100 dark:border-slate-800">
                                        <span className="font-mono text-[10px] md:text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">CÓDIGO: {item.codigo}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="w-full md:w-[55%] p-3 md:p-6 flex flex-col gap-3 md:gap-6 bg-white dark:bg-slate-900">
                            <div className="flex flex-col items-center gap-3 md:gap-4">
                                ...
                                <div className="flex items-center gap-3 md:gap-6">
                                    ...
                                    <div className="w-28 h-16 md:w-32 md:h-20 flex items-center justify-center bg-white dark:bg-slate-900 border-4 border-amber-500/20 dark:border-amber-500/10 rounded-2xl md:rounded-3xl shadow-inner group">
                                        <span className={cn(
                            ...
                            <div className="w-full max-w-[260px] md:max-w-xs mx-auto">
                                <NumericKeypad 
                                    onPress={handleKeyPress}
                                    onBackspace={handleBackspace}
                                    onClear={handleClear}
                                    className="[&>button]:h-11 md:[&>button]:h-16 [&>button]:rounded-xl md:[&>button]:rounded-2xl [&>button]:text-lg md:[&>button]:text-xl"
                                />
                            </div>
